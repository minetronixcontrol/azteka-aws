    ...
    if(req.params.value == 'true'){
        try {
            await Viaje.findOneAndUpdate(
                { _id: id, [`asiento_${asiento}`]: 'false' },
                { [`asiento_${asiento}`] : 'true' }
            );
            let apartado = await Apartado.find({idVendedor: idVendedor, idViaje: idViaje, asiento: asiento});
        ...
        try {
            await Viaje.findOneAndUpdate(
                { _id: id, [`asiento_${asiento}`]: 'true' },
                { [`asiento_${asiento}`] : 'false' }
            );
            let apartado = await Apartado.find({idVendedor: idVendedor, idViaje: idViaje, asiento: asiento});
            if(apartado.length > 0){
                await Apartado.findByIdAndRemove(apartado[0]._id);
                return res.json({status:'ASIENTO DESAPARTADO!!!'});
            }
        } catch (error) {
            return res.json(error);
        }
